...
import { SonarCloudQueryModal } from './SonarCloudQueryModal';
import { type SonarCloudQuery, useDeleteSonarCloudQuery } from '@/hooks/api';
import { toast } from 'sonner';
import { ConfirmationModal } from '@/components/ui/confirmation-modal';

interface SonarCloudQueryTableProps {
...
export function SonarCloudQueryTable({
  queries,
  isLoading,
  isQueryModalOpen,
  onQueryModalOpenChange,
}: SonarCloudQueryTableProps) {
  const [editData, setEditData] = useState<SonarCloudQuery | undefined>(
    undefined,
  );
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [deleteData, setDeleteData] = useState<SonarCloudQuery | undefined>(
    undefined,
  );
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const deleteQuery = useDeleteSonarCloudQuery();

  // Handle edit button click
  const handleEditClick = (query: SonarCloudQuery) => {
    setEditData(query);
    setIsEditModalOpen(true);
  };

  // Handle delete button click
  const handleDeleteClick = (query: SonarCloudQuery) => {
    setDeleteData(query);
  ...
  const queryColumnHelper = createColumnHelper<SonarCloudQuery>();
  const queryColumns = [
    queryColumnHelper.accessor('name', {
      header: 'Name',
      cell: info => info.getValue(),
    }),
    queryColumnHelper.accessor('project', {
      header: 'Project',
      cell: info => info.getValue(),
    }),
    queryColumnHelper.accessor('metric', {
      header: 'Metric',
      cell: info => (
  ...
  if (queryTable.getRowModel().rows.length === 0) {
    return (
      <div className="text-center py-8 border border-dashed border-gray-300 rounded-lg">
        <p className="text-gray-500 mb-4">No SonarCloud queries found</p>
